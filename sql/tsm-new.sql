...
SELECT T.hours, T.weekending
FROM timesheettbl T
WHERE (SUM(T.hours) > 168)
ORDER BY T.weekending

SELECT  SUM(T.hours) as totalhours, T.weekending
FROM timesheettbl T
WHERE (weekending = date('2017-11-10'));

SELECT  DISTINCT T.weekending
FROM timesheettbl T
ORDER BY weekending ASC

SELECT  SUM(T.hours) as totalhours, SUM(E.rate * T.hours) as totalamount 
FROM timesheettbl T
LEFT JOIN employeetbl E on E.id = T.employeeid
WHERE (weekending = date('2017-10-13'));

UPDATE `timesheettbl` SET status=1 WHERE 1
